...
import type { ImportMap, RawImportMap } from '../types';

var importMapPromise = Promise.resolve();
export var importMap: ImportMap = { imports: {}, scopes: {}, depcache: {}, integrity: {} };
...
function processScripts () {
  [].forEach.call(document.querySelectorAll('script'), function (script: HTMLScriptElement) {
    ...
    if (script.type === 'systemjs-module') {
      ...
      var fetchPromise = script.src ? (System.fetch || fetch)(script.src, { integrity: script.integrity, priority: script.fetchPriority, passThrough: true }).then(function (res) {
        if (!res.ok)
          throw Error(process.env.SYSTEM_PRODUCTION ? res.status as any : 'Invalid status code: ' + res.status);
        return res.text();
      }).catch(function (err) {
...
function extendImportMap (importMap: ImportMap, newMapText: string, newMapUrl: string) {
  var newMap: RawImportMap = {};
  try {
