 ...
 */
import { global, hasSymbol } from './common';
import { errMsg } from './err-msg';
import type { ImporterSetter, LoadRecord, ModuleNamespace, Registration, SystemJSLoader } from './types';
...
var systemJSPrototype: SystemJSLoader = SystemJS.prototype;
...
systemJSPrototype.import = function (id, parentUrl, meta) {
  ...
  .then(function() {
    return loader.resolve(id, parentUrl as string | undefined, meta);
  })
  .then(function (id) {
...
function loadToId (load: LoadRecord): string {
  return load.id;
}
function triggerOnload (loader: SystemJSLoader, load: LoadRecord, err: any, isErrSource?: boolean) {
  loader.onload(err, load.id, load.d && load.d.map(loadToId), !!isErrSource);
  if (err)
    throw err;
}

var lastRegister: Registration | undefined;
systemJSPrototype.register = function (deps, declare, metas) {
  lastRegister = [deps, declare, metas] as Registration;
};
...
export function getOrCreateLoad (loader: SystemJSLoader, id: string, firstParentUrl?: string, meta?: any): LoadRecord {
  var load: LoadRecord = loader[REGISTRY][id];
  if (load)
    return load;

  var importerSetters: Array<ImporterSetter | undefined> = [];
  var ns: ModuleNamespace = Object.create(null);
  if (toStringTag)
  ...
  .then(function (registration): [string[], Array<ImporterSetter | undefined>, any[]] {
    if (!registration)
      throw Error(errMsg(2, process.env.SYSTEM_PRODUCTION ? id : 'Module ' + id + ' did not instantiate'));
    function _export (name: string | ModuleNamespace, value?: any) {
...
function instantiateAll (loader: SystemJSLoader, load: LoadRecord, parent: LoadRecord, loaded: { [id: string]: boolean }): Promise<any> | undefined {
  if (!loaded[load.id]) {
    loaded[load.id] = true;
...
function topLevelLoad (loader: SystemJSLoader, load: LoadRecord): Promise<ModuleNamespace> {
...
function postOrderExec (loader: SystemJSLoader, load: LoadRecord, seen: { [id: string]: boolean }): Promise<any> | undefined | void {
  if (seen[load.id])
    return;
  seen[load.id] = true;
  ...
  var depLoadPromises: Array<Promise<any>> | undefined;
...
global.System = new (SystemJS as any)();
