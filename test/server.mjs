...
if (
  testName.startsWith('test-chrome') &&
  process.env.CI_BROWSER &&
  !process.env.CI_BROWSER.includes('chrome')
)
  process.exit(0);
...
http
  .createServer(async function (req, res) {
    // Helps CI debugging:
    if (process.env.CI_BROWSER) console.log('REQ: ' + req.url);
    setBrowserTimeout();
    if (req.url.startsWith('/done')) {
      ...
      if (shouldExit) {
        if (spawnPs) spawnPs.kill('SIGKILL');
