suite('SystemJS Standard Tests', function () {
  ...
  test('import.meta.resolve contextual package maps', function () {
    return System.import('fixtures/resolve.js')
      .then(function (m) {
        return m.resolve(
          'maptest',
          baseURL + 'fixtures/browser/scope-test/index.js',
        );
      })
      .then(function (resolved) {
        assert.equal(
  ...
  if (typeof WebAssembly !== 'undefined' && typeof process === 'undefined')
    test('Loading WASM over 4KB limit', function () {
      return System.import('fixtures/wasm/addbloated.wasm').then(function (m) {
  ...
  test('should handle css modules with urls without quotes', function () {
    return System.import(
      'fixturesbase/css-modules/url-without-quotes.css',
    ).then(function (m) {
      assert.ok(m);
      assert.ok(isCSSStyleSheet(m.default));
      assert.equal(
        m.default.cssRules[0].cssText,
        '.hello { background-image: url("http://localhost:8080/test/fixtures/css-modules/path/to/image.png"); }',
      );
      assert.equal(
        m.default.cssRules[1].cssText,
        '.world { background-image: url("http://localhost:8080/test/fixtures/css-modules/path/to/image.png"); }',
      );
      assert.equal(
        m.default.cssRules[2].cssText,
        'body { background-image: url("http://localhost:8080/test/fixtures/css-modules/path/to/image.png"); }',
      );
      document.adoptedStyleSheets = document.adoptedStyleSheets.concat(
        m.default,
      );
  ...
  test('should throw when trying to load an HTML module', function () {
    return System.import('/test/test.html').then(
      function () {
        throw Error(
  ...
  test('should not get confused by filenames in url hash when resolving module type', function () {
    return System.import('fixturesbase/css-modules/hash.css?foo=bar.html').then(
      function (m) {
        assert.ok(m);
        assert.ok(isCSSStyleSheet(m.default));
      },
  ...
  test('should allow deletion of modules that failed to instantiate', function () {
    return System.import('fixtures/link-error.js').then(
      ...
      function () {
        ...
        assert.ok(
          System.delete(System.resolve('fixtures/link-error-child.js')),
        );
        assert.ok(System.delete(System.resolve('fixtures/not-found.js')));
        assert.ok(
          System.delete(System.resolve('fixtures/link-error-child2.js')),
        );
      },
    );
  });
